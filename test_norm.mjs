...
const match = serverTs.match(/const qaidaOverrides\s*=\s*(\{[\s\S]*?\n\s*\});/);
let qaidaOverrides = {};
if (match) {
...
const missing = [];
for (const word of userList) {
  const normBase = normalizeArabicText(word);
  const normDiac = normalizeArabicWithDiacritics(word);
  ...
  if (!found) missing.push({word, normBase, normDiac});
}
