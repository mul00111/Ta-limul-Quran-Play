...
with open('src/components/HuroofMaddahGameModal.tsx', 'r') as f:
    maddah_content = f.read()

# I want to extract JUST the functional component `HuroofMaddahGameModal`
match = re.search(r'(export const HuroofMaddahGameModal.*?)$', maddah_content, re.DOTALL)
component_code = match.group(1)

# Replace all maddah with tashdeed inside the component
component_code = component_code.replace('HuroofMaddahGameModal', 'TashdeedGameModal')
component_code = component_code.replace('MaddahGameItem', 'TashdeedGameItem')
component_code = component_code.replace('maddah_game_score', 'tashdeed_game_score')
component_code = component_code.replace('maddah_game_coins', 'qaida_coins')
component_code = component_code.replace('maddah_best_streak', 'tashdeed_best_streak')
component_code = component_code.replace('onBack', 'onClose')
component_code = component_code.replace('WORD_MADDAH_GAME_ITEMS', 'TASHDEED_GAME_ITEMS')
component_code = component_code.replace('SINGLE_MADDAH_GAME_ITEMS', 'TASHDEED_GAME_ITEMS')
component_code = component_code.replace('TRIPLET_MADDAH_GAME_ITEMS', 'TASHDEED_GAME_ITEMS')
component_code = component_code.replace('ALL_MADDAH_MASHQ_WORDS', 'TASHDEED_GAME_ITEMS')

component_code = component_code.replace('w.maddahType', 'w.category')
component_code = component_code.replace('maddahType:', 'category:')
component_code = component_code.replace('hijjaText', 'urduSpelling')
component_code = component_code.replace('meaningOrContext', 'tajweedNote')

component_code = component_code.replace('صوتی و بصری گیم: حروفِ مدہ 🎯', 'صوتی و بصری گیم: تشدید 🎯')
component_code = component_code.replace('بَا، بُوْ، بِيْ جیسے تمام حروف مدہ اور کلمات کی صوتی و بصری مشق', 'تشدید والے کلمات کی صوتی و بصری مشق اور تفریحی گیمز')
component_code = component_code.replace('مفردات مدہ (۱ حرف)', 'آسان سطح (Basic)')
component_code = component_code.replace('ثلاثی جوڑیاں', 'درمیانی سطح (Medium)')
component_code = component_code.replace('قرآنی و مشقی کلمات', 'مشکل سطح (Hard)')

tashdeed_data = """import React, { useState, useEffect, useRef, useMemo } from 'react';
...
with open('src/components/TashdeedGameModal.tsx', 'w') as f:
    f.write(tashdeed_data + component_code)
